...
%矩阵运算
    ...
    tril_a = tril(a,1);%tril(矩阵，主对角线上当第k条斜线)生成矩阵的下三角阵，trio上三角阵
    %加、减、乘、乘方
        a*a
    %点运算
...
%矩阵的修改
    ...
    %指定维数拼接
        ...
        c2_a = cat(2,a,a); %水平拼接
    % *变维
        rs_a = reshape(a,1,9); %元素个数不变，矩阵变维m*n
%%
%信息获取
    %矩阵的行列数
...
%多维数组
    %创建
        ...
        %cat法
            mul_31 = [1,2,3;2,3,4];
            mul_32 = [3,4,5;4,5,6];
            mul_3 = cat(3,mul_31,mul,32); %把a1a2按照“3”维连接
%%
% *字符串
    %创建
        str0 = 'hello world!';  %单引号引起
        str1 = 'I''m your father!'; %字符串中单引号写两遍
        str3 = ['I''m''your''father'];  %方括号链接多字符串
        str4 = strcat(str0, str1);  %strcat连接字符串函数
        str5 = strvcat(str0, str1); %strvcat连接产生多行字符串
        str6 = doubke(str0);    %取str0的ASCII值，也可用abs函数
    ...
    % 操作
        ...
        %查找替换
            strfind(str0, str1);    %再str0找到str1的位置
            strmatch(str1, str0);   %再str0字符串数组中找到str1开头的行数
            strtok(str0);   %截取str0第一个分隔符（空格，tab，回车）前的部分
            strrep(str0, str1, str2);   %再str0中用str2替换str1
        %其他
...
%%
%转换
    % ___2___ --> 如mun2str，将数字转字符串；dec2hex，将十进制转十六进制
    str_b = num2str(b);
    % abs, double取ACSII码；char把ASCII转字符串
    abs_str = abs('aAaA');
